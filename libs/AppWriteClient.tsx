...
export const APPWRITE_CONFIG = {
  ...
  userCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_PROFILE || "67f225fc0022b2dc0881",
  trackCollectionId:
  ...
  statisticsCollectionId:
    ...
    "67fa8893001cb797a064",
  analyticsCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_TRACK_ANALYTICS ||
    "67fa89b40009e6b07992",
  interactionsCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_TRACK_INTERACTIONS ||
    "67fa8aa1001ba183beb4",

  // Добавление коллекций для вайбов
  vibesCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_VIBE_POSTS || "67f22eb700352cd35cd8",
  vibeCommentsCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_VIBE_COMMENTS ||
    "67f2307500286019354c",
  vibeLikesCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_VIBE_LIKES || "67f22ff30010feacfd1b",

  // Добавление коллекций для миксов
  mixPostsCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_MIX_POSTS || "6857da72002661f6e89b",
  mixLikesCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_MIX_LIKES || "mix_likes",

  // Добавление коллекций для новостей
  newsCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_NEWS || "68285239000757a15e82",
  newsLikesCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_NEWS_LIKE || "67db665e002906c5c567",

  // Для обратной совместимости с старыми ключами
  friendsCollectionId:
    process.env.NEXT_PUBLIC_COLLECTION_ID_FRIENDS || "67f22b92000e12316e52",
};
...
try {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      // Add global error handler for unhandled realtime errors
      window.addEventListener("unhandledrejection", (event) => {
        if (event.reason && event.reason.toString().includes("realtime")) {
          console.error(
            "[REALTIME] Unhandled promise rejection:",
            event.reason,
          );
      ...
      console.error(
        "[REALTIME] Failed to set up realtime listeners:",
        realtimeError,
      );
    }
  }
...
export const safeCreateFile = async (
  ...
  try {
    ...
    try {
      ...
      const result = await nodeStorage.createFile(bucketId, fileId, {
        path: tempFilePath,
        filename: file.name,
      } as any);
